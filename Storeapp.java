            ...
            while (makingPurchase) { // Loop for product selection and adding to cart
                ...
                while (!response.equalsIgnoreCase("EXIT") && !response.equalsIgnoreCase("N")) {
                    ...
                    if (response.equalsIgnoreCase("Y")) {
                        makingPurchase = true; // Continue with the next product selection
                        break;
                    } else if (response.equalsIgnoreCase("EXIT")) {
                        System.out.println("Thank you for checking out our store. Please come back again.");
                        return; // Exit the program
                    } else if (response.equalsIgnoreCase("N")) {
                        makingPurchase = false; // Proceed to checkout
                        break;
                    }
                }
                ...
                if (!makingPurchase) {
                    ...
                    if (purchase.equalsIgnoreCase("Y")) {
                        ...
                        while (attempts < 3) {
                            ...
                            try {
                                ...
                                if (cash >= total) {
                                    ...
                                    // After payment is successful, ask for a new transaction or exit
                                    String newTransactionResponse = "";
                                    while (!newTransactionResponse.equalsIgnoreCase("N") && !newTransactionResponse.equalsIgnoreCase("Y")) {
                                        System.out.println("Do you want to make another transaction? Y/N");
                                        newTransactionResponse = reader.next();

                                        if (newTransactionResponse.equalsIgnoreCase("Y")) {
                                            newTransaction = true; // Restart the transaction process
                                            break; // Goes back to Step 1
                                        } else if (newTransactionResponse.equalsIgnoreCase("N")) {
                                            System.out.println("Goodbye! Thank you for shopping with us.");
                                            return; // Exit the program
                                        }
                                    }
                                    break; // Exit payment loop after successful payment
                                } else {
                        ...
                        makingPurchase = true; // Go back to making purchases
                    } else {
